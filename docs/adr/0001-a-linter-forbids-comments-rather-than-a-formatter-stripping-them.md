...
This decision was first taken in Too Many Chests, a sibling mod in this family, and is recorded there as its ADR 0034. That repository is not public, so this record deliberately carries the whole argument rather than deferring to it, and the reasoning above stands on its own. More Doors adopted it the same day, as its ADR 0013. The configuration, the wiring and the self-test below are the same in each, so the family converges on one solution rather than several.
...
- **The unit tests resolve a newer Guava than the mod runs against.** Checkstyle on the test classpath lifts Guava from Minecraft's 32.1.2-jre to 33.2.0-jre, and `failureaccess` from 1.0.1 to 1.0.2, on `testRuntimeClasspath` only. Around thirty further artifacts join that configuration, and everything else resolves upward to versions already present. Nothing reaches a published configuration.

The exposure is not negligible here, and it is worth stating rather than waving through. Forty-one of the ninety-one test classes import `net.minecraft`, and twenty-one of them bootstrap the real registries through `MinecraftBootstrapExtension`, which runs `Bootstrap.initialize()` under the lifted Guava and swallows `Throwable` on every step. A Guava incompatibility during bootstrap would therefore be absorbed silently and the suite would still pass against a half-initialised registry. That swallowing predates this change and is not introduced by it, but it is what makes the version difference harder to notice than it looks. Pinning Guava down for `testRuntimeClasspath` would risk breaking the Checkstyle the self-test runs, so the difference is accepted and recorded instead.
...
- **The gate is duplicated rather than shared.** Each mod carrying this rule holds its own copy of `config/checkstyle/checkstyle.xml`, its own self-test and its own fixture, so a change to the rule has to be made in each repository. A shared convention plugin would fix that and is not worth its own build for one rule.
- **The self-test is named for a rule rather than a class.** [`../standards.md`](../standards.md) asks that unit tests map one to one onto classes. `NoCommentsRuleTest` covers a Checkstyle configuration, which is not a class in this source tree, so it departs from that rule deliberately. The alternative names it after nothing a reader would search for.
...
- **The wider family is still inconsistent.** [`../standards.md`](../standards.md) names five mods at its head, and this repository was the only one of them carrying no copy of it until now. Too Many Chests, More Doors and this repository each enforce the rule with the same gate, Too Many Chests first. More Arrows and Teleport Effects hold byte-identical copies of the standard stating the opposite rule, that comments explain why when the reason is genuinely surprising, and neither enforces anything. Loot Lock is a sixth repository, named in that document as a canonical implementation but absent from its family list, and carries no copy at all. Because each repository's copy describes its own enforcement, the supposedly shared document now states different facts in different repositories, which is the drift it exists to prevent. Reconciling the document with itself, and the remaining repositories with the document, is separate work and is not attempted here.
