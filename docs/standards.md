...
These standards apply to every mod in this family: Dogs Unleashed, More Arrows, More Doors, Teleport Effects, and Too Many Chests. They exist because the same decisions kept being re-made from memory.
...
- **Optional integrations degrade cleanly.** EMI and JEI are suggested dependencies, and Mod Menu is supported without being declared as one. The mod loads and runs correctly with none of them installed, and no class referencing their types loads when they are absent.
...
- **The source carries no comments at all.** Not explanatory ones, not javadoc, not "why" ones. Naming and structure carry the meaning, and anything that genuinely needs explaining is either a decision record or a sign the code should be reshaped until it does not. Dogs Unleashed enforces this with Checkstyle rather than trusting memory: a single `MatchXpath` rule over the parsed comment nodes fails the build at `file:line` on any Java source in any source set, javadoc included, and never edits source to do it. A unit test runs that rule against a fixture of every comment form and every comment-like string literal, so the gate cannot silently stop gating. Build scripts and property files are outside it and keep their comments. See [ADR 0001](adr/0001-a-linter-forbids-comments-rather-than-a-formatter-stripping-them.md) for why a linter does this and a formatter must not. Too Many Chests and More Doors run the same gate, recorded in their own ADRs 0034 and 0013; Too Many Chests took the decision first.
