---
name: geo-prop
description: Author a small GeckoLib prop for Dogs Unleashed, its box-UV texture and optional animation, and review it without launching the game. Use when an item or block needs a real 3D model rather than a flat sprite.
---
...
M=src/client/resources/assets/dogs-unleashed/geo/<prop>.geo.json
T=src/client/resources/assets/dogs-unleashed/textures/item/<prop>.png
...
1. `src/client/resources/assets/dogs-unleashed/geo/<prop>.geo.json`
2. `src/client/resources/assets/dogs-unleashed/textures/item/<prop>.png`
...
5. `src/main/resources/assets/dogs-unleashed/models/item/<prop>.json` as
   `{"parent": "builtin/entity"}`. No flat sprite: the renderer draws the GUI too.
6. `BuiltinItemRendererRegistry.INSTANCE.register(...)` in `DogsUnleashedClient`.
