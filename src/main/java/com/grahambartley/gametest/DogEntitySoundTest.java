...
public final class DogEntitySoundTest implements FabricGameTest {
  ...
  public void huskyHasNoBarkSoundRegistered(TestContext context) {
    context.assertFalse(
        Registries.SOUND_EVENT.containsId(Identifier.of("dogs-unleashed", "entity.husky.bark")),
  ...
  private <T extends UnleashedDogEntity> void testDogDoesNotBarkWhenLowHealth(
    ...
    context.runAtTick(
        ...
        () -> {
          context.assertTrue(
              dog.getBarkCooldownTicks() == 0, "Husky should not bark at low health");
          context.complete();
        });
  }
  ...
  private <T extends UnleashedDogEntity> void testDogDoesNotBarkWhenHasTarget(
    ...
    context.runAtTick(
        ...
        () -> {
          context.assertTrue(
              dog.getBarkCooldownTicks() == 0, "Husky should not bark with a target");
          context.complete();
        });
  }
  ...
  private <T extends UnleashedDogEntity> void testDogDoesNotBarkWhenTakingDamage(
    ...
    context.runAtTick(
        ...
        () -> {
          context.assertTrue(
              dog.getBarkCooldownTicks() == 0, "Husky should not bark when taking damage");
          context.complete();
        });
  }
  ...
  private <T extends UnleashedDogEntity> void testBarkCooldownStaysInactive(
    ...
    context.runAtTick(
        10,
        () -> {
          context.assertTrue(dog.getBarkCooldownTicks() == 0, "Husky bark cooldown should stay 0");
          context.complete();
        });
  }
}
